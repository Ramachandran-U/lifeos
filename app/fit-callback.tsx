...
export default function FitCallbackScreen() {
  ...
  useEffect(() => {
    if (Platform.OS !== 'web' || typeof window === 'undefined') {
      setStatus('error');
      setError('Google Fit sign-in is only supported on web right now.');
      return;
    }
    ...
    handleFitCallback(code, clientId)
      .then(() => {
        setStatus('ok');
        setTimeout(() => router.replace('/(tabs)/health'), 500);
      })
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: c.background }]}>
      <View style={styles.container}>
        {status === 'working' && (
          <>
            <LoadingDots />
            <Heading style={[styles.title, { color: c.textPrimary }]}>Connecting Google Fit...</Heading>
        ...
        {status === 'ok' && (
          <>
            ...
            <Caption style={{ color: c.textMuted }}>Redirecting to Health...</Caption>
          </>
        )}
        {status === 'error' && (
          <>
