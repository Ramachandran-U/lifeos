...
export default function CalendarCallbackScreen() {
  ...
  useEffect(() => {
    if (Platform.OS !== 'web' || typeof window === 'undefined') {
      setStatus('error');
      setError('Google Calendar sign-in is only supported on web right now.');
      return;
    }
    ...
    handleCalendarCallback(code, clientId)
      .then(() => {
        setStatus('ok');
    ...
    <SafeAreaView style={[styles.root, { backgroundColor: c.background }]}>
      <View style={styles.container}>
        {status === 'working' && (
          <>
            <LoadingDots />
            <Heading style={[styles.title, { color: c.textPrimary }]}>Connecting Google Calendar...</Heading>
        ...
        {status === 'ok' && (
          <>
            ...
            <Caption style={{ color: c.textMuted }}>Redirecting to Today...</Caption>
          </>
        )}
        {status === 'error' && (
          <>
