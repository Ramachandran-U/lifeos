...
All AI calls go through [`src/ai/client.ts`](../src/ai/client.ts) → the `workers/ai-proxy` Cloudflare Worker (`POST /claude`, Bearer = Supabase JWT, `task` field forwarded for per-feature rate-limit bucketing) → the configured LLM (Anthropic Claude / Google Gemini / OpenAI — switch via `LLM_PROVIDER` Worker env var). The chatbot (`app/chat.tsx`) and voice assistant (Gemini Live, `src/ai/voiceClient.ts`) share the same auth/transport. Every function:
