...
describe('Button3D', () => {
  it('renders its title', () => {
    render(<Button3D title="Start" onPress={() => {}} />);
    expect(screen.getByText('Start')).toBeTruthy();
  });

  it('calls onPress when pressed', () => {
    const onPress = jest.fn();
    render(<Button3D title="Go" onPress={onPress} />);
  ...
  it('does not fire onPress when disabled', () => {
    const onPress = jest.fn();
    render(<Button3D title="Go" onPress={onPress} disabled />);
    fireEvent.press(screen.getByText('Go'));
  ...
  it('shows loadingTitle and blocks presses while loading', () => {
    const onPress = jest.fn();
    render(<Button3D title="Save" loadingTitle="Saving…" loading onPress={onPress} />);
  ...
  it('falls back to title when loading without a loadingTitle', () => {
    render(<Button3D title="Save" loading onPress={() => {}} />);
    expect(screen.getByText('Save')).toBeTruthy();
  });

  it('renders each tone without crashing', () => {
    const tones = ['primary', 'goal', 'health', 'finance', 'career', 'social', 'polymath', 'success', 'danger'] as const;
