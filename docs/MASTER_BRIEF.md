...
| 3 | **Financial Goal Engine** | AI-generated savings/investment plans; auto-ingests bank transactions from Gmail (HDFC/ICICI/Axis) and categorises with a rules→local-ML→AI pipeline |
...
2. **AI-native from day one.** An LLM is wired into every meaningful decision — goal decomposition, routine generation, skill-gap analysis, food photo recognition, blood report parsing, transaction categorisation, weekly insights. The model is chosen per task by a tiered router (Gemini is the current default provider, with a server-side fallback chain), so the provider can change without touching the product.
3. **Private by design.** Sensitive data (health logs, food entries, blood reports, on-device chat) lives on-device only — SQLite on native, IndexedDB / localStorage on web. A Cloudflare Worker proxies AI calls (so no LLM provider key ever ships in the bundle) and a Supabase project handles auth + opt-in anonymous telemetry. Telemetry is OFF by default and uses a random device id with no PII; users can disable it and export or delete all data from Settings.
...
| AI | Per-task tiered model routing via the `ai-proxy` Worker — Gemini default provider, server-side fallback chain (Groq/Anthropic); Gemini Live for voice |
