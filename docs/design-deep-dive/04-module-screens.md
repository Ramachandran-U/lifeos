...
> **Open AC5 conflict for ruling:** `SubscriptionsBillsCard.formatDueLabel` can render the literal `Overdue` on Finance from real data while AC5 bans the string screen-wide; §3.5 item 6 commits internals-unchanged, so it stands until ruled. Likewise `12-WEEK EXECUTION PLAN` (amendment l) awaits the same ruling pass.
