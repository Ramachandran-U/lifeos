...
> **Amendment (founder ruling, 2026-06-14 — the violet voice sweep; extends §A.4's five-surface policy app-wide):**
> Violet means THE BRAND OR THE AI IS SPEAKING — never "this is a button" (Button/Button3D lost their silent `primary` defaults; tone is now a required prop), never a selection state (tab bar, segmented controls, chips → ink selected-states), never gamification (which speaks xp gold / streak ember / badge amber). **The `xp` token itself left the violet family**: `#9D8CFF`→`#FFD60A` dark, `#5B4FE8`→`#8A5800` light (the §A.5 V5 'violet on xp' clause is retired; contrast matrix passes unmodified). Founder judgment calls: tab-bar neutral tabs select in ink (domain tabs keep their hues); onboarding day-screens are domain-tinted (discovery flow + day1-routine stay violet — the AI/planner genuinely speaks there); the companion keeps violet (the AI made visible). The two cross-domain synthesis visualizations — the HexRadar polygon and the LifeScore ring — keep violet as the brand's view across all six domains. The AI's review/transparency pages carry ONE violet attribution each. ~80 surfaces re-toned; enforced by the new Guard E (`violetVoiceCompliance.test.ts`, 51-file voice allowlist, one-way ratchet). index.tsx's frozen flag-off branch and `src/screens/legacy/` ride untouched to their 2026-06-27 deletion.
