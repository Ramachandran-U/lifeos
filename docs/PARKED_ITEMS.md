...
| 13.2 🅿️ | **Surface "Disconnect Google Fit" in the flag-on Health tree.** The connected ConnectRow's press target is Sync (§3.0.3 copy table), leaving no home for Disconnect — the legacy tree keeps it, the V1 tree does not surface it (spec 04 amendment i). Candidates: a Fit detail sheet off the row's long-press, or a row in notifications/data settings. Filed by W4 batch A. | The V1 tree is dogfood-only while the flag is off-by-default; legacy (with Disconnect) remains every user's path. | **Must land before `module_hierarchy_v1` graduates to 100%** — a user on the V1 tree has no way to revoke Fit. |
