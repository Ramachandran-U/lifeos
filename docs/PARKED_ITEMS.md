...
| 1.2 🅿️ | **Native (iOS/Android) build with the current code.** Web is deployed; native is not. | No native build/submit has been run for the P1 work. | Before any device validation of sync/backup, and before the App Store/Play track. Note **T10's schema-drift fixes are native-only** (SQLite `ADD COLUMN`s) — latent until a native build exists. |
...
| 1.5 🅿️ | **Repo hygiene:** gitignore `supabase/.branches/` + `supabase/.temp/`; harden `scripts/post-export-web.js` font-rename against Windows EPERM (the rename is already wrapped in try/catch, so it's cosmetic). | Low priority; doesn't block anything. | Any time; quick. |
